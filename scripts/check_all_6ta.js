...
async function run() {
  ...
  try {
    ...
    console.log("--- CHECKING ALL PARTICIPATIONS vs HISTORIAL ---");
    ...
    console.log(`Found ${parts.rows.length} participations`);
    ...
    `, [fechaTorneoId, categoriaId]);
    console.log(`Found ${history.rows.length} history entries`);
    ...
    parts.rows.forEach(p => {
      ...
      if (!h) {
        console.log(`WARNING: Player ${p.nombre} ${p.apellido} (ID ${p.jugador_id}) has participation but NO history entry.`);
      } else {
        if (p.part_puntos !== h.hist_puntos) {
          console.log(`DISCREPANCY: Player ${p.nombre} ${p.apellido} (ID ${p.jugador_id})`);
          console.log(`  Participacion: ${p.part_puntos} pts (${p.part_instancia})`);
          console.log(`  Historial:     ${h.hist_puntos} pts (${h.motivo})`);
          discrepancies++;
        }
      }
    });
